...
async function handleGenerate(root: string, options: RulesOptions) {
    let ides = options.ide?.split(",").map(s => s.trim());
    ...
    if (ides) {
        ...
        // Auto-include companion targets (e.g. antigravity → antigravity-dir)
        if (ides.includes("antigravity") && !ides.includes("antigravity-dir")) {
            ides.push("antigravity-dir");
        }
    }
    ...
    console.log(chalk.gray("IDE-local files (gitignored): .cursor/, .gemini/, .trae/, .warp/, MCP configs"));
}
